...
class DallasComponent : public PollingComponent {
 public:
  // Set the pins used for one wire communication.
  void set_pin(InternalGPIOPin pin_or_pins) {
  	InternalGPIOPin *pins = pin_or_pins;
  
  	if (pins[1] == nullptr) {  // Is a single pin
    	this->pin_ = pin_or_pins; 
  	} else { // Is an array of two pins
    	this->input_pin_ = pins[0];
    	this->output_pin_ = pins[1];
  	}
  }
  // Add a sensor to this Dallas hub.
