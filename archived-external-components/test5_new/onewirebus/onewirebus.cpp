...
bool HOT IRAM_ATTR OneWireBus::reset() {
  if (this->rmt_mode_ && get_esp32()) {
    // RMT mode, ESP32
    rmt_enable_tx_carrier(this->rmt_channel_);
    delayMicroseconds(480);   // tx_continuous_carrier
    rmt_disable_tx_carrier(this->rmt_channel_);
      
    // delay 70us
    delayMicroseconds(75);  
      
    // Check bus level - if low, device present  
    return !this->input_pin_.digital_read();
      
  } else if (this->split_io_) {
    // Dual pin bitbang
    ...
    // Set pin to input and check level 
