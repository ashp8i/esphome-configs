...
#include "onewire_temperature_sensor.h"
...
namespace esphome
{
  namespace onewire
  {
    ...
    {
      ESP_LOGCONFIG(TAG, "Setting up OnewireComponent...");
      one_wire_->searchReset();
      ESP_LOGI(TAG, "1-Wire using pin %d", pin_->get_pin());
      OneWire::Id id;
      one_wire_->searchReset();
      while (one_wire_->search(id, false) == OneWire::EC_MORE)
      {
        uint64_t address;
        memcpy(&address, &id[0], sizeof(address));
        ESP_LOGI(TAG, "Found 1-Wire device 0x%s", format_hex(address).c_str());
    ...
    {
      ESP_LOGCONFIG(TAG, "OnewireComponent:");
    ...
    void OnewireComponent::update()
    {
      status_clear_warning();
      OneWire::ErrorCode result = one_wire_->reset();
