...
namespace esphome
{
    namespace shellydallasng
    {
        ...
        class ShellyDallasNgTemperatureSensor : public sensor::Sensor
        {
        public:
            void set_parent(ShellyDallasNgComponent *parent)
            {
                parent_ = parent;
            }
            ...
            bool setup()
            {
                auto sensor = DSTherm(*parent_->one_wire_output_);
            ...
            {
                auto sensor = DSTherm(*parent_->one_wire_input_);
                Placeholder<DSTherm::Scratchpad> scratchpad;
                auto result = sensor.readScratchpad(id_, &scratchpad);
                if (result != OneWireNg::EC_SUCCESS)
                {
        ...
        private:
            ShellyDallasNgComponent *parent_;
            OneWireNg::Id id_;
            uint64_t address_;
