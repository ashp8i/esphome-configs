...
void OneWireBusComponent::setup() {
  ...
  // Clear bus with 480μs high
  this->pin_->pin_mode(gpio::FLAG_INPUT | gpio::FLAG_PULLUP);  
  delayMicroseconds(480);

  this->bus_ = new OneWireBus(this->pin_);    

  // Search bus and store addresses
  std::vector<uint64_t> addresses = this->bus_->search();
  this->addresses_ = addresses;  

  // Add devices 
  for (auto &address : addresses) {
    OneWireBusDevice *device = new OneWireBusDevice(this, address); 
...
size_t OneWireBusDevice::get_device_count() {
  return this->devices_.size();
}

uint64_t OneWireBusDevice::get_device_address(size_t index) {
  return this->devices_[index];
}

void OneWireBusDevice::register_timeout(std::function<void()> callback) {
  // Register a timeout to call the callback function
}

void OneWireBusDevice::dump_config() {
  ESP_LOGD("onewirebus", "Found %d 1-Wire devices:", this->devices_.size());
  for (size_t i = 0; i < this->devices_.size(); i++) {
    ESP_LOGD("onewirebus", "  0x%016llX", this->devices_[i]);
  }
}
