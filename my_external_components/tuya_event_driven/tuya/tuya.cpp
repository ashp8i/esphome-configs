...
void Tuya::setup() {
  ...
  this->protocol_version_ = 0xFF;
  this->product_ = "";
  this->status_pin_reported_ = -1;
  this->reset_pin_reported_ = -1;
  this->mcu_requires_heartbeat_ = false;
  this->mcu_requires_wifi_status_ = false;
  this->skip_handshake_ = false;

  // Start with ONE heartbeat - no intervals yet!
  this->send_empty_command_(TuyaCommandType::HEARTBEAT);

  ESP_LOGI(TAG, "Tuya component started - smart event-driven mode");
}
...
void Tuya::loop() {
  // PURE EVENT-DRIVEN: Process UART data ONLY
  while (this->available()) {
    uint8_t c;
    this->read_byte(&c);
    this->handle_char_(c);
...
void Tuya::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Product: '%s'", this->product_.c_str());
  ESP_LOGCONFIG(TAG, "  Skip handshake: %s", this->skip_handshake_ ? "true" : "false");
  ESP_LOGCONFIG(TAG, "  MCU requires heartbeat: %s", this->mcu_requires_heartbeat_ ? "true" : "false");
  ESP_LOGCONFIG(TAG, "  MCU requires WiFi status: %s", this->mcu_requires_wifi_status_ ? "true" : "false");
...
void Tuya::handle_char_(uint8_t c) {
  ...
  // Event: Check if we have a complete message
  if (this->validate_message_()) {
    // Event: Process the complete message
    this->process_complete_message_();
    this->rx_message_.clear();
  }
}

void Tuya::process_complete_message_() {
  if (this->rx_message_.size() < 7) return;
  ...
  const uint8_t *buffer = &this->rx_message_[6];

  // Event: Handle the command
  this->handle_command_(command, version, buffer, length);
}
...
void Tuya::handle_command_(uint8_t command, uint8_t version, const uint8_t *buffer, size_t len) {
  ESP_LOGD(TAG, "EVENT: Received command 0x%02X (version %u)", command, version);

  // Event: Update protocol version if detected
  if (this->protocol_version_ == 0xFF) {
    this->protocol_version_ = version;
    ESP_LOGI(TAG, "Protocol auto-detected: v%u", version);
  }

  switch (command) {
    case TuyaCommandType::HEARTBEAT: {
      ESP_LOGD(TAG, "EVENT: Heartbeat received");

      // Event: Detect if MCU requires periodic heartbeats
      if (!this->mcu_requires_heartbeat_) {
        this->mcu_requires_heartbeat_ = true;
        ESP_LOGI(TAG, "MCU requires periodic heartbeats - setting up 15s interval");
        this->set_interval("heartbeat", 15000, [this] {
          this->send_empty_command_(TuyaCommandType::HEARTBEAT);
        });
      }

      // Event: Complete initialization if needed
      if (this->init_state_ == TuyaInitState::INIT_HEARTBEAT) {
        if (this->skip_handshake_) {
          // Skip handshake - go straight to done
          ESP_LOGI(TAG, "Skipping handshake as requested");
          this->init_state_ = TuyaInitState::INIT_DONE;
          this->initialized_callback_.call();
          this->dump_config();
        } else {
          this->init_state_ = TuyaInitState::INIT_PRODUCT;
          ESP_LOGD(TAG, "EVENT: Sending product query");
          this->send_empty_command_(TuyaCommandType::PRODUCT_QUERY);
        }
      }
      break;
    ...
    case TuyaCommandType::PRODUCT_QUERY: {
      ESP_LOGD(TAG, "EVENT: Product query response");

      // Event: Parse product data
      std::string product_data(reinterpret_cast<const char *>(buffer), len);
      this->product_ = product_data;
      ESP_LOGI(TAG, "EVENT: Product data received: %s", product_data.c_str());

      // Event: Move to next state
      if (this->init_state_ == TuyaInitState::INIT_PRODUCT) {
        this->init_state_ = TuyaInitState::INIT_CONF;
        ESP_LOGD(TAG, "EVENT: Sending config query");
        this->send_empty_command_(TuyaCommandType::CONF_QUERY);
      }
      break;
    }
    ...
    case TuyaCommandType::CONF_QUERY: {
      ESP_LOGD(TAG, "EVENT: Config query response");

      // Event: Parse config data
      if (len >= 2) {
        this->status_pin_reported_ = buffer[0];
        this->reset_pin_reported_ = buffer[1];
        ESP_LOGI(TAG, "EVENT: Config received - status pin: %d, reset pin: %d",
                 this->status_pin_reported_, this->reset_pin_reported_);
      }

      // Event: Check if MCU requires WiFi status updates
      if (!this->mcu_requires_wifi_status_) {
        // Check if device requested WiFi status
        if (this->status_pin_reported_ == -1) {
          this->mcu_requires_wifi_status_ = true;
          ESP_LOGI(TAG, "MCU requires periodic WiFi status - setting up 1s interval");
      ...
      // Event: Complete initialization
      this->init_state_ = TuyaInitState::INIT_DONE;
      ESP_LOGI(TAG, "EVENT: Initialization complete!");
      this->initialized_callback_.call();
      this->dump_config();
      break;
    }
    ...
    case TuyaCommandType::WIFI_STATE: {
      ESP_LOGD(TAG, "EVENT: WiFi state requested");

      // Event: Send WiFi status immediately
      this->send_command_(TuyaCommand{.cmd = TuyaCommandType::WIFI_STATE,
                                      .payload = {this->get_wifi_status_code_()}});
      break;
    }

    case TuyaCommandType::DATAPOINT_REPORT_SYNC:
    case TuyaCommandType::DATAPOINT_REPORT_ASYNC: {
      ESP_LOGD(TAG, "EVENT: Datapoint update received");

      // Event: Parse datapoint data
      this->handle_datapoints_(buffer, len);

      // Event: Send ACK if needed
      if (command == TuyaCommandType::DATAPOINT_REPORT_SYNC) {
        this->send_command_(TuyaCommand{.cmd = TuyaCommandType::DATAPOINT_REPORT_ACK, .payload = {0x01}});
      }
      break;
    }

    default: {
      ESP_LOGD(TAG, "EVENT: Unknown command 0x%02X", command);
      break;
    }
  }
}
...
void Tuya::set_skip_handshake(bool skip) {
  this->skip_handshake_ = skip;
  ESP_LOGI(TAG, "Handshake skip option set to: %s", skip ? "true" : "false");
}
